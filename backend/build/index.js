...
var matchInit = function(ctx, logger, _nk, params) {
  let mode = "classic";
...
var matchJoin = function(_ctx, logger, nk, dispatcher, _tick, state, presences) {
  const s = state;
...
var matchSignal = function(_ctx, logger, _nk, _dispatcher, _tick, state, data) {
  const s = state;
  if (data === "end_match") {
    s.status = "GAME_OVER";
...
var matchTerminate = function(_ctx, logger, _nk, dispatcher, _tick, state, graceSeconds) {
  const s = state;
  logger.info(
